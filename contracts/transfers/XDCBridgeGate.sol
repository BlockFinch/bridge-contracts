...
import "../interfaces/IXDCBridgeToken.sol";
    ...
    IXDCBridgeGate
{
    ...
    /// @dev Address of IXDCBridgeTokenDeployer contract
    ...
    /// @dev Maps xbridgeId (see getXbridgeId) => bridge-specific information.
    mapping(bytes32 => XbridgeInfo) public getXbridge;
    /// @dev Maps xbridgeId (see getXbridgeId) => fee information
    mapping(bytes32 => XbridgeFeeInfo) public getXbridgeFeeInfo;
    ...
    /// @dev Maps xbridgeId (see getXbridgeId) to threshold amount after which
    ...
    function send(
        ...
        (amountAfterFee, xbridgeId, feeParams) = _send(
            _permit,
            _tokenAddress,
            _amount,
            _chainIdTo,
    ...
    function claim(
        ...
        _checkConfirmations(submissionId, _xbridgeId, _amount, _signatures);
    ...
    {
        bytes32 xbridgeId = getXbridgeId(getChainId(), _tokenAddress);
        if (!getXbridge[xbridgeId].exist) revert XbridgeNotFound();
        ...
        getXbridgeFeeInfo[xbridgeId].collectedFees += paid;
    ...
    function deployNewAsset(
        ...
        bytes32 xbridgeId = getbXbridgeId(_nativeChainId, _nativeTokenAddress);

        if (getXbridge[xbridgeId].exist) revert AssetAlreadyExist();

        bytes32 deployId = getDeployId(xbridgeId, _name, _symbol, _decimals);

        // verify signatures
        // ISignatureVerifier(signatureVerifier).submit(deployId, _signatures, excessConfirmations);

        address deBridgeTokenAddress = IXDCBridgeTokenDeployer(deBridgeTokenDeployer)
            .deployAsset(xbridgeId, _name, _symbol, _decimals);

        _addAsset(xbridgeId, deBridgeTokenAddress, _nativeTokenAddress, _nativeChainId);
    }
    ...
    function updateAssetFixedFees(
        ...
        XbridgeFeeInfo storage xbridgeFee = getXbridgeFeeInfo[_xbridgeId];
        for (uint256 i = 0; i < _supportedChainIds.length; i++) {
            xbridgeFee.getChainFee[_supportedChainIds[i]] = _assetFeesInfo[i];
        }
    }
    ...
    function updateAsset(
        ...
        XbridgeInfo storage xbridge = getXbridge[_xbridgeId];
        // don't check existence of xbridge - it allows to setup asset before first transfer
        xbridge.maxAmount = _maxAmount;
        xbridge.minReservesBps = _minReservesBps;
        getAmountThreshold[_xbridgeId] = _amountThreshold;
    }
    ...
    function setXDCBridgeTokenDeployer(address _deBridgeTokenDeployer) external onlyAdmin {
    ...
    /// @inheritdoc IXDCBridgeGate
    function withdrawFee(bytes32 _xbridgeId) external override nonReentrant onlyFeeProxy {
        XbridgeFeeInfo storage xbridgeFee = getXbridgeFeeInfo[_xbridgeId];
        // Amount for transfer to treasury
        uint256 amount = xbridgeFee.collectedFees - xbridgeFee.withdrawnFees;

        if (amount == 0) revert NotEnoughReserves();

        xbridgeFee.withdrawnFees += amount;

        if (_xbridgeId == getXbridgeId(getChainId(), address(0))) {
            _safeTransferETH(feeProxy, amount);
        } else {
            // don't need this check as we check that amount is not zero
            // if (!getXbridge[_xbridgeId].exist) revert XbridgeNotFound();
            IERC20Upgradeable(getXbridge[_xbridgeId].tokenAddress).safeTransfer(feeProxy, amount);
        }
        emit WithdrawnFee(_xbridgeId, amount);
    }
        ...
        nonReentrant
    {
        ...
        if (!xbridge.exist) revert XbridgeNotFound();
        uint256 minReserves = (xbridge.balance * xbridge.minReservesBps) / BPS_DENOMINATOR;

        if (minReserves + _amount > IERC20Upgradeable(_tokenAddress).balanceOf(address(this)))
            revert NotEnoughReserves();

        xbridge.lockedInStrategies += _amount;
        ...
        nonReentrant
    {
        ...
        if (!xbridge.exist) revert XbridgeNotFound();
        xbridge.lockedInStrategies -= _amount;
        IERC20Upgradeable(xbridge.tokenAddress).safeTransferFrom(
            defiController,
            address(this),
            _amount
        );
    ...
    function _checkConfirmations(
        ...
        ISignatureVerifier(signatureVerifier).submit(
            _submissionId,
            _signatures,
            _amount >= getAmountThreshold[_xbridgeId] ? excessConfirmations : 0
        );
    }
    ...
    function _addAsset(
        ...
        if (xbridge.exist) revert AssetAlreadyExist();
        if (_tokenAddress == address(0)) revert ZeroAddress();

        xbridge.exist = true;
        xbridge.tokenAddress = _tokenAddress;
        xbridge.chainId = _nativeChainId;
        // Don't override if the admin already set maxAmount in updateAsset method before
        if (xbridge.maxAmount == 0) {
            xbridge.maxAmount = type(uint256).max;
        }
        // set minReservesBps to 100% to prevent using new asset by DeFiController by default
        xbridge.minReservesBps = uint16(BPS_DENOMINATOR);
        if (getAmountThreshold[_xbridgeId] == 0) {
            getAmountThreshold[_xbridgeId] = type(uint256).max;
        }
    ...
    function _send(
        ...
        if (_amount > xbridge.maxAmount) revert TransferAmountTooHigh();

        //_processFeeForTransfer
        {
            DiscountInfo memory discountInfo = feeDiscount[msg.sender];
            XbridgeFeeInfo storage xbridgeFee = getXbridgeFeeInfo[xbridgeId];
            ...
            if (_useAssetFee) {
                if (_tokenAddress == address(0)) {
                    ...
                    assetsFixedFee = xbridgeFee.getChainFee[_chainIdTo];
                ...
                bytes32 nativeXbridgeId = getXbridgeId(getChainId(), address(0));
                getXbridgeFeeInfo[nativeXbridgeId].collectedFees += nativeFee;
            ...
            xbridgeFee.collectedFees += totalFee;
        ...
        if (isNativeToken) {
            xbridge.balance += amountAfterFee;
        }
        else {
            xbridge.balance -= amountAfterFee;
            IXDCBridgeToken(xbridge.tokenAddress).burn(amountAfterFee);
        }
        return (amountAfterFee, xbridgeId, feeParams);
    }
    ...
    function _publishSubmission(
...

        {
            emit MonitoringSendEvent(
                submissionId,
                nonce,
                getXbridge[_xbridgeId].balance,
                IERC20Upgradeable(getXbridge[_xbridgeId].tokenAddress).totalSupply()
            );
        }
    ...
    function _claim(
        ...
        XbridgeInfo storage xbridge = getXbridge[_xbridgeId];
        if (!xbridge.exist) revert XbridgeNotFound();
        isNativeToken = xbridge.chainId == getChainId();

        if (isNativeToken) {
            xbridge.balance -= _amount + _autoParams.executionFee;
        } else {
            xbridge.balance += _amount + _autoParams.executionFee;
        }

        address _token = xbridge.tokenAddress;
        ...
        emit MonitoringClaimEvent(
            _submissionId,
            xbridge.balance,
            IERC20Upgradeable(xbridge.tokenAddress).totalSupply()
        );
    }
    ...
    function _mintOrTransfer(
        ...
        if (_amount > 0) {
            if (isNativeToken) {
                ...
                IXDCBridgeToken(_token).mint(_receiver, _amount);
            }
        }
    }
    ...
    */
    ...
    function getDefiAvaliableReserves(address _tokenAddress)
        ...
        returns (uint256)
    {
        XbridgeInfo storage xbridge = getXbridge[getXbridgeId(getChainId(), _tokenAddress)];
        return (xbridge.balance * (BPS_DENOMINATOR - xbridge.minReservesBps)) / BPS_DENOMINATOR;
    }
    ...
    /// @param _chainId Current chain id.
    /// @param _tokenAddress Address of the asset on the other chain.
    function getbXbridgeId(uint256 _chainId, bytes memory _tokenAddress) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_chainId, _tokenAddress));
    }

    /// @inheritdoc IXDCBridgeGate
    function getXbridgeChainAssetFixedFee(
        bytes32 _xbridgeId,
        uint256 _chainId
    ) external view override returns (uint256) {
        // if (!getXbridge[_xbridgeId].exist) revert XbridgeNotFound();
        return getXbridgeFeeInfo[_xbridgeId].getChainFee[_chainId];
    }
    ...
    /// @param _xbridgeId Id of an asset, see getXbridgeId.
